...
def plot_degreepostpred_sparse(G, objmcmc, folder_name, ndraws=5000):
    ...
    for i in range(ndraws):
        ...
        [Gsamp, _, _, _, _, _, _, _, _, _] = ggp_dyngraphrnd_sparse('BFRY', alpha, sigma, tau, T, phi,
                                                                                 rho, gvar, settings)


        for t in range(T):
            [_, _, freq_samp[t, i, :]] = plot_degree_sparse(Gsamp[t])


    for t in range(T):
        [_, centerbins1[t,:], freq_true[t,:]] = plot_degree_sparse(G[t])
...
def plot_degreepostpred_sparse_chains(G, objmcmc, folder_name, ndraws=5000):
    ...
    for i in range(nchains):
        ...
        for key in list(objmcmc['samples'].keys()):
            if key in samples:
                samples[key] = np.concatenate((samples[key], objmcmc['samples'][key][i]), axis=-1)
            else:
                samples[key] = objmcmc['samples'][key][i]
    nsamples = samples['w'].shape[2]
    T = samples['w'].shape[1]
    ind = [math.floor(n) for n in np.linspace(0, nsamples-1, num=ndraws)]
    settings = objmcmc['settings']
...
def plot_degreepostpred_real_counts(N_new, objmcmc, folder_name, ndraws=5000):
    ...
    for i in range(ndraws):
        ...
        [Gsamp, _, _, _, Nnew_samp, _, _, M_samp, _, _] = ggp_dyngraphrnd_sparse('BFRY', alpha, sigma, tau, T, phi,
                                                                                 rho, gvar, settings)


        for t in range(T):
            [_, _, freq_samp[t, i, :]] = plot_degree_sparse(Nnew_samp[t])


    for t in range(T):
        [_, centerbins1[t,:], freq_true[t,:]] = plot_degree_sparse(N_new[t])
...
def plot_degreepostpred_real_chains(G, objmcmc, folder_name, ndraws=5000):
...
def plot_degreepostpred_real_counts_chains(N_new, objmcmc, folder_name, ndraws=5000):
