...
def ggp_dyngraphrnd(typeweights, alpha, sigma, tau, T, phi, rho, gvar, settings):
    ...
    if typeweights == "GGP":  # power law with exp sigma
        ...
        indmax = 100000 # so that to allocate memory
    ...
    N_new = np.zeros((T, size_w, size_w),dtype='int')
    N_old = np.zeros((T, size_w, size_w),dtype='int')
    N = np.zeros((T, size_w, size_w),dtype='int')
    Z = np.zeros((T, size_w, size_w),dtype='int')
    ...
    N[0,:,:] = N_new[0,:,:]
    Z[0,:,:] = (np.squeeze(N[0,:,:])+ np.transpose(np.squeeze(N[0,:,:])))>0
    ...
    for t in range(1,T):
        ...
        N_old[t,:,:] = np.random.binomial(N[t-1,:,:], np.exp(-rho*dt) )

        # Aggregate old + new
        N[t,:,:] = N_new[t,:,:] + N_old[t,:,:]

        # Obtain undirected graph from directed one
        Z[t, :, :] = (np.squeeze(N[t, :, :]) + np.transpose(np.squeeze(N[t, :, :]))) > 0


    for t in range(T):
        M[:, t] = np.transpose(np.sum(np.squeeze(N_new[t, :, :]),0)) + np.sum(np.squeeze(N_new[t, :, :]), 1) - np.diag(np.squeeze(N_new[t, :, :]))
    #     keyboard
